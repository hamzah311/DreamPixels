...
var Razorpay = (function () {
  ...
  function Razorpay() {
    ...
    // ✅ [UPDATED] Added environment variable fallback for key_id/key_secret
    var key_id =
        options.key_id || process.env.RAZORPAY_KEY_ID || process.env.RAYZORPAY_KEY_ID, // fallback for common typo
      key_secret =
        options.key_secret ||
        process.env.RAZORPAY_KEY_SECRET ||
        process.env.RAYZORPAY_KEY_SECRET,
      oauthToken = options.oauthToken || process.env.RAZORPAY_OAUTH_TOKEN,
      headers = options.headers;

    // ✅ [UPDATED] Added more descriptive error message
    if (!key_id && !oauthToken) {
      throw new Error(
        '`key_id` or `oauthToken` is mandatory. (Hint: check .env values RAZORPAY_KEY_ID and RAZORPAY_KEY_SECRET)'
      );
    }
